...
DATASET_DIR = Path("Plant Village Dataset")
PLOTS_DIR = Path("plant village detection plots")
...
def train_model():
    ...
    # --- Hyperparameters ---
    BATCH_SIZE = 32
    EPOCHS = 8            # Early stopping governs, not this cap
    ...
    for epoch in range(EPOCHS):
        ...
        # Fine-Tuning Phase: Unfreeze last 3 blocks at epoch 3
        ...
        if avg_val_loss < best_val_loss:
            ...
            early_stop_counter = 0
            model_path = MODELS_DIR / f'plant_village_model_{RUN_TIMESTAMP}.pth'
            torch.save(model.state_dict(), model_path)
            print(f"Saved new best model checkpoint: plant_village_model_{RUN_TIMESTAMP}.pth")
        else:
    ...
    model.load_state_dict(torch.load(MODELS_DIR / f'plant_village_model_{RUN_TIMESTAMP}.pth', map_location=device))
    model.eval()
    all_preds_best = []
    all_labels_best = []
    with torch.no_grad():
